...
class Employee extends AFWObject{
        ...
        public static function loadAndUpdateFromExternalHRSystem($username, $employee_num)
        {
                ...
                if($ok and $resEmployee['company_id'] and $resEmployee['email'])
                {
                        ...
                        if(!$resEmployee["guest"]) $employee->updateMeFromJson($resEmployee);
                }
                else
                {
        ...
        {
           $obj = new Employee();
           if($id_sh_org) $obj->select("id_sh_org",$id_sh_org);
        ...
        public function getDisplay($lang="ar")
        {
               ...
               if($this->getVal("g_f_firstname")) $data[] = $this->getVal("g_f_firstname");
               $data[] = $this->getVal("lastname");
               
               $disp = trim(implode(" ",$data));
               
               if(!$disp)  {
                  if($this->getId()>0) $disp = "موظف ".$this->getId();
                  else $disp = "...";
               }
        ...
        public function beforeMaj($id, $fields_updated) 
        {
            ...
            global $lang;
                ...
                if((!$this->getVal("idn_type_id")) and $this->getVal("idn"))
                {
                    ...
                    if($idn_correct) $this->set("idn_type_id",$idn_type_id);
                }
        ...
        */
        ...
        protected function importRecord($dataRecord,$orgunit_id,$overwrite_data,$options,$lang,$dont_check_error)
        {
          ...
          if($overwrite_data or $employee->is_new)
          {
                 ...
                 if($genre) list($val_ok, $val_parsed_or_error) =  AfwStringHelper::parseAttribute($employee,"genre_id",$genre,$lang); else $val_ok = true;
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($nationality) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"country_id",$nationality,$lang); else $val_ok = true;
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($firstname) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"firstname",$firstname,$lang); else $val_ok = true;
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($fatherfirstname) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"f_firstname",$fatherfirstname,$lang); else $val_ok = true;
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($grandfathername) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"g_f_firstname",$grandfathername,$lang); else $val_ok = true;
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($lastname) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"lastname",$lastname,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 
                 if($jobname) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"job",$jobname,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($mobile) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"mobile",$mobile,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($email) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"email",$email,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($employee_num) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"employee_num",$employee_num,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;

                 if($dep_id) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"dep_id",$dep_id,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
                 
                 if($dep_name) list($val_ok, $val_parsed_or_error) = AfwStringHelper::parseAttribute($employee,"dep_name",$dep_name,$lang); else $val_ok = true; 
                 if(!$val_ok) $errors[] = $val_parsed_or_error;
    ...
    public function isFromOurCompany()
    {
        ...
        public function beforeDelete($id,$id_replace) 
        {
            ...
            {   
               ...
               if($id_replace==0)
               {
                   // FK part of me - not deletable 
                        ...
                        */
                        ...
                        if(!$simul) $this->execQuery("update ${server_db_prefix}hrm.orgunit set id_responsible='$id_replace' where id_responsible='$id' ");
               ...
               else
               {
                        ...
                        // crm.request-المنسق المكلف	employee_id  أنا تفاصيل لها-OneToMany
                        if(!$simul) $this->execQuery("update ${server_db_prefix}crm.request set employee_id='$id_replace' where employee_id='$id' ");
                        
                        // crm.response-الموظف المكلف بالرد	employee_id  أنا تفاصيل لها-OneToMany
                        if(!$simul) $this->execQuery("update ${server_db_prefix}crm.response set employee_id='$id_replace' where employee_id='$id' ");
      ...
      protected function getPublicMethods()
      {
        ...
        public function updateMyUserInformation($lang="ar", $from_ldap="", $commit=true, $force_reset_pwd_for_user=false)
        {
               ...
               if(!$from_ldap) $from_ldap = $ldap_use;
               ...
               if(!$email)
               {
                    ...
                    if($orgunit)
                    {
                        ...
                        if((!AfwStringHelper::stringStartsWith($sh_code,"@")) and (!AfwStringHelper::stringStartsWith($sh_code,"."))) $sh_code = "@".$sh_code; 
                        $email = $username.$sh_code;
                    }
                    else return array("can't update user information without email and without company email-domain defined","");
               }
               ...
               if($this->getVal("address")) $usr->set("address",$this->getVal("address"));
               if($this->getVal("city_id")) $usr->set("city_id",$this->getVal("city_id"));
               if($this->getVal("mobile")) $usr->set("mobile",$this->getVal("mobile"));
               
               
               $usr->set("username",$this->getVal("username"));
               
               $usr->commit();
               if($usr->is_new or $force_reset_pwd_for_user) list($errors_arr[], $infos_arr[]) = $usr->initUser($from_ldap);
        ...
        {
           if($fgroup=="employment") return array("name"=>$fgroup, "css"=>"pct_50");
           if($fgroup=="communication") return array("name"=>$fgroup, "css"=>"pct_50");
        ...
        public function beLongsTo($orgunit_id, $recursive=false)        
        {
            ...
            if(!$recursive) return ($this->getVal("id_sh_div") == $orgunit_id); 
